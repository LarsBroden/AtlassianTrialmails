...
## D18 — Filter tightening discovered via local rehearsal: exclude bugcrowdninja.com + non-prospect company name patterns

**Choice:** Two additions to the day-1 classifier in `lib/marketplace.ts`:
1. Treat `@bugcrowdninja.com` as Atlassian-internal (joins `@atlassian.com` and `*.atlassian.com` under the same `atlassian-internal` reject reason). Bugcrowd is Atlassian's bug-bounty platform; researchers there install Marketplace apps to look for vulnerabilities, not to evaluate them.
2. New `non-prospect-company` reject reason matching company-name patterns: contains `bugbounty`, contains `sandbox`, ends in `-test`, or ends in `-internal`. Word-boundary anchored to avoid false positives on real company names like "Testify" or "Internal Plumbing".

**How it was found:** the 2026-05-27 local rehearsal (DRY_RUN against the real Marketplace API for vendor 38407) returned 408 EVALUATION licenses from the last 30 days. The would-send list contained 15 candidates, of which **7 were `@bugcrowdninja.com` accounts** and **3 had explicit `bugbounty-test-*`, `*-sandbox-*`, or `*-internal` tenant names**. Without these filters, every bug-bounty researcher and every Atlassian-internal test tenant would receive a personalised welcome email. After the additions, the would-send count dropped from 15 → 5, with the remaining 5 looking like legitimate prospects (Himax Technologies, CR Group/Rambus, plus a few small/solo evaluators).

**Tests:** 6 new tests in `tests/marketplace.test.ts` cover each new path including a regression test that real company names with `test`/`internal` substrings (e.g. `Testify`, `TestCorp`, `Acme Tester`) do NOT match. Total suite now 65 tests (was 59).

**Tuning surface for future:** the patterns live as a top-level const `NON_PROSPECT_COMPANY_PATTERNS` in `lib/marketplace.ts`. If real prospects start being false-positived, append patterns there.

## D19 — Accept degraded rendering in Outlook Classic / Word-engine clients

**Choice (2026-05-27):** Don't rewrite the email templates for Outlook compatibility. Ship the current modern-CSS design and accept that Outlook Classic / Outlook for Microsoft 365 desktop (both still on the Word rendering engine) will show a degraded but functional version.
**Context:** After the local rehearsal Lars sent himself the day-1 template via Outlook desktop. The rendering was visibly worse than the design intent: hero gradient → solid colour, resource cards stacked instead of 2×2 grid (no CSS Grid), step rows misaligned (no flexbox), Google Fonts → system fallback, decorative blob circles disappeared (no `position: absolute`), `gap` ignored.
**Why these clients break:** Outlook Classic and Outlook for Microsoft 365 desktop on Windows render mail via the Word engine, which lacks support for `linear-gradient`, `display: grid`, `display: flex`, `gap`, `@media` queries, `position: absolute`, external font `@import`, and partially-supported `border-radius`. New Outlook for Windows (WebView2-based), Outlook on the Web, Outlook for Mac, mobile, and Gmail all render the template correctly.
**Reasoning:** A bulletproof rewrite (nested `<table>` layout + VML for Outlook backgrounds + inline-only styles + system fonts) is 2–4 hours of work and would make the email look more conservative across all clients — including the ones where it currently looks great. Net-net not a clear win for the audience mix (which trends toward Cloud-native admins on web mail). The email body, links, CTAs, trust message, and signature all still come through legibly in the degraded view — only the visual polish is lost.
**Reversal trigger:** if recipient feedback or open-rate data ever indicates the Outlook desktop rendering is hurting conversion, revisit by rewriting with a bulletproof-email framework (e.g. MJML) or by hand-coding to a known-good email template pattern.
